    ...
    axios
      ...
      .catch((err) => {
        if (err.response?.data?.errors)
          for (const val of err.response.data.errors) alert(val);
  ...
  return (
    <main className="h-screen w-screen m-0 p-0 flex justify-center items-center bg-[#000000ed]">
      <div className="w-[30%] bg-[#232329] rounded-lg shadow flex flex-col justify-center items-center gap-5 p-4 text-white font-mono">
        <h1 className="text-center text-3xl font-semibold text-[#c1bfff]">
          Create Account
        </h1>
        ...
        <div className="w-full flex flex-col justify-center items-center gap-2">
          ...
          <span>
            ...
            <Link
              href="/signin"
              className="text-[#7a76ff] underline hover:text-sky-600"
            >
              Signin
            </Link>
          </span>
